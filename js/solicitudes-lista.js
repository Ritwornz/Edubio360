...
function solicitudesVisibles() {
    return obtenerSolicitudes().filter(function (solicitud) {
        if (modoOrientador) return solicitud.orientador === sesionSolicitudes?.correo || (!solicitud.orientador && solicitud.estado === "PENDIENTE");
...
function renderizarSolicitudes() {
    ...
    document.querySelectorAll(".reprogramar-solicitud").forEach(function (boton) {
        boton.addEventListener("click", function () {
            const fecha = boton.parentElement.querySelector("input").value;
            const hoy = new Date().toISOString().slice(0, 10);
            if (!fecha || fecha < hoy) return;
            actualizarSolicitud(Number(boton.dataset.id), { estado: "REPROGRAMADA", fecha });
            crearNotificacion(sesionSolicitudes.correo, `Reprogramación solicitada para ${fecha}.`);
            renderizarSolicitudes();
        });
    });
}
