...
if (!oferta) {
    ...
    contenedorDetalle.innerHTML = `<article class="detalle-oferta"><div><img src="${oferta.imagen}" alt="${oferta.carrera}" class="detalle-imagen"></div><div class="detalle-contenido"><span class="badge">${oferta.grupoComparacion}</span><h1>${oferta.carrera}</h1><h2>${oferta.institucion}</h2><div class="detalle-datos"><p><strong>Sede:</strong> ${oferta.sede}</p><p><strong>Modalidad:</strong> ${oferta.modalidad}</p><p><strong>Jornada:</strong> ${oferta.jornada}</p><p><strong>Duración:</strong> ${oferta.duracion}</p><p><strong>Matrícula:</strong> ${formatearPrecio(oferta.matricula)}</p><p><strong>Arancel:</strong> ${formatearPrecio(oferta.arancel)}</p><p><strong>Cupos disponibles:</strong> ${oferta.cupos}</p>${oferta.cupoCritico !== null && oferta.cupos <= oferta.cupoCritico ? '<p class="alerta-cupos" role="status">Últimos cupos disponibles</p>' : ""}</div><button type="button" class="boton boton-primario" id="btnComparar">Agregar a mi selección</button><a href="catalogo.html?carrera=${encodeURIComponent(oferta.grupoComparacion)}" class="boton boton-claro">Ver instituciones comparables</a><a href="solicitud.html?oferta=${oferta.id}" class="boton boton-secundario">Solicitar orientación</a><a href="mapa.html?sede=${encodeURIComponent(oferta.sede)}" class="boton boton-claro">Ver sede en mapa</a></div></article>`;
